...
from capabilities.canonical_runtime import DISPATCH, generate_tests, validation
...
def test_registry_generated_ids_are_above_reserved_range():
    ...
    assert [c["id"] for c in data["capabilities"] if c.get("canonical")==True] == [f"CAP-{i:03d}" for i in range(1,11)]
