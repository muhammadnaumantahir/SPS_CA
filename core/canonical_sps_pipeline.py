...
class CanonicalSPSPipeline:
    ...
    def run_submission(self, *, user_request: str, code: str, language: str, file_path: str = "", target_project: Optional[str] = None) -> Dict[str, Any]:
        ...
        knowledge = self.knowledge.build_snapshot(
            language=language, file_path=file_path, symbols=(), capabilities=(),
            facts={"experience_count": len(experience.tasks), "working_source_chars": len(code)},
        )
        ...
        adaptation_params, adaptation_changes = self.adaptation.adjust_parameters(
            {"language": language, "timeout_seconds": 5.0},
            {"target_language": language, "complex": len(code) > 4000},
        )
        ...
        pipeline = self._build_pipeline(
            result=result, brain_intent=brain_intent, knowledge_valid=knowledge_valid,
            experience=experience, failure_patterns=failure_patterns,
            reusable_capabilities=reusable_capabilities, adaptation_changes=adaptation_changes,
