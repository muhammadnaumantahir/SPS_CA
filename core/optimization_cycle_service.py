...
class OptimizationCycleService:
    ...
    def __init__(
        ...
        *,
        ...
        long_term_path: str = DEFAULT_LONG_TERM_PATH,
    ) -> None:
    ...
    def assess_after_task(self, capability_ids: Iterable[str]) -> OptimizationCyclePlan:
        ...
        state["long_term_learning"] = {
            "path": str(self.long_term.path),
            "updated_at": learning.get("updated_at"),
            "total_tasks": learning.get("total_tasks", 0),
            "overall_success_rate": learning.get("overall_success_rate", 0.0),
    ...
    def prepare_evolution_action(self, plan: OptimizationCyclePlan, *, language: str, task_description: str) -> EvolutionActionPlan:
        ...
        state = self._load_state()
        state.update({"last_plan": plan.to_dict(), "last_action_plan": action.to_dict(), "execution_authority": self._authority_dict()})
        self._save_state(state)
        return action
    ...
    def prepare_evolution_candidates(self, plan: OptimizationCyclePlan, *, language: str, task_description: str) -> list[dict[str, Any]]:
        ...
        state = self._load_state()
        state["evolution_candidates"] = [{"cycle_id": item["cycle_id"], "source_capability_id": item["source_capability_id"], "reason": item["reason"], "plan": item["plan"].__dict__} for item in candidates]
        self._save_state(state)
        return candidates
    ...
    def execute_authorized_action_plan(self, action: EvolutionActionPlan, *, project_root: str = ".") -> list[dict[str, Any]]:
        ...
        state["last_execution_authorization"] = {
            "cycle_id": action.cycle_id, "authorized": allowed, "reason": reason,
            "candidate_count": len(candidates), "recorded_at": datetime.now(timezone.utc).isoformat(),
        }
        if not allowed:
            outcomes = [EvolutionCycleOutcome(
                cycle_id=action.cycle_id,
                capability_id=str(item["plan"].capability_id),
                source_capability_id=item.get("source_capability_id", ""),
                authorized=False, executed=False, result={"reason": reason},
            ).to_dict() for item in candidates]
        ...
        for candidate in candidates:
            ...
            outcome = EvolutionCycleOutcome(
                cycle_id=candidate["cycle_id"], capability_id=str(result.get("capability_id") or candidate["plan"].capability_id),
                source_capability_id=candidate.get("source_capability_id", ""), authorized=True, executed=True,
                promoted=bool(result.get("promoted")), rolled_back=bool(result.get("rolled_back")), result=result,
            )
    ...
    def execute_candidate(self, candidate: dict[str, Any], *, project_root: str = ".") -> dict[str, Any]:
        ...
        transaction = EvolutionTransaction(
            project_root,
            transaction_id=str(candidate.get("cycle_id") or plan.capability_id),
            registry_path="capabilities/registry.json",
        )
        try:
            transaction.begin(["capabilities/registry.json"])
            result = self.evolution.develop_capability_for_gap(plan, project_root=project_root)
            if result.get("promoted"):
                transaction.commit()
            else:
                transaction.rollback()
        except (Exception, EvolutionTransactionError):
            try:
                transaction.rollback()
            finally:
                raise
        ...
        history.append({"cycle_id": candidate.get("cycle_id", ""), "source_capability_id": candidate.get("source_capability_id", ""), "result": result})
