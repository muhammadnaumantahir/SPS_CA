...
def run_suite(path: str | Path, *, model: str = "", live_evolve: bool = False, max_scenarios: int | None = None, results_dir: str | Path = DEFAULT_RESULTS_DIR, record_feedback: bool = True) -> dict[str, Any]:
    ...
    for index, scenario in enumerate(scenarios, 1):
        request = str(scenario.get("request", "")).strip(); scenario_id = str(scenario.get("id", f"scenario-{index:03d}")); feedback = scenario.get("feedback")
        turn = pipeline.run_submission(user_request=request, code=str(scenario.get("code", "")), language=str(scenario.get("language", "python")), file_path=str(scenario.get("filename", "main.py")), source="scenario", run_id=run_id, scenario_id=scenario_id, feedback=str(feedback or ""))
    ...
    output["finished_at"] = datetime.now(timezone.utc).isoformat(); output["pass_rate"] = round(output["passed"] / output["total"], 4) if output["total"] else 0.0; output["generated_capabilities_at_end"] = _generated_capability_count(); output["evolution_events"] = len(evolution.list_events(200)); (root / f"{run_id}.json").write_text(json.dumps(output, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"); (root / "latest.json").write_text(json.dumps(output, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"); return output
