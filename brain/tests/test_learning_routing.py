"""Integration tests for evidence-aware Brain routing."""
...
def test_evidence_qualified_generated_capability_can_replace_seeded_default():
    response = json.dumps({"language": "python", "language_confidence": 0.95, "intent_class": "code_modification", "intent": "add validation", "reasoning": "explicit source modification requested", "steps": [{"capability_id": "CAP-002", "reason": "canonical default"}]})
    experience = [
    ...
    catalog = [
        ...
        {"id": "CAP-011", "name": "Generated Safe Modification", "status": "active", "generated": True, "intent_class": "code_modification", "allowed_intents": ["code_modification"], "forbidden_intents": ["test_generation"]},
    ]
    brain = Brain(provider=FakeProvider(response), model="test-model")
    plan = brain.plan(request="Add input validation to this function", code="def add(a, b):\n    return a + b\n", language="python", file_path="main.py", capability_catalog=catalog, experience_context=experience)
