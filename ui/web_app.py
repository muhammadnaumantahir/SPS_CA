...
REGISTRY_PATH=str(ROOT/"capabilities"/"registry.json"); EXPERIENCE_PATH=str(ROOT/"experience"/"logs"/"experience_log.json"); SESSIONS_PATH=ROOT/"runtime"/"sessions.json"; EVOLUTION_PATH=ROOT/"runtime"/"evolution_events.json"
sessions=SessionStore(SESSIONS_PATH); evolution=EvolutionEvidenceStore(EVOLUTION_PATH,REGISTRY_PATH)

def service_for(model:str="")->SpsAssistantService: return SpsAssistantService(registry_path=REGISTRY_PATH,experience_path=EXPERIENCE_PATH,model=model)
...
def extract_prompt_code(request:str)->tuple[str,str]:
    ...
    lang, code=matches[0]; return code.strip(), lang.strip().lower()
class Handler(BaseHTTPRequestHandler):
    server_version="SPS-CA/3.1"
    def _send(self,status:int,payload:Any,content_type:str="application/json")->None:
        body=payload if isinstance(payload,bytes) else json.dumps(payload,ensure_ascii=False).encode() if content_type=="application/json" else str(payload).encode(); self.send_response(status); self.send_header("Content-Type",f"{content_type}; charset=utf-8"); self.send_header("Content-Length",str(len(body))); self.send_header("Cache-Control","no-store"); self.end_headers(); self.wfile.write(body)
    def _json_body(self)->dict[str,Any]:
        length=int(self.headers.get("Content-Length","0")); data=json.loads(self.rfile.read(length) or b"{}"); return data if isinstance(data,dict) else {}
    def do_GET(self)->None:
        path=urlparse(self.path).path; routes={"/":(ROOT/"ui"/"web"/"index.html","text/html"),"/static/app.js":(ROOT/"ui"/"web"/"app.js","text/javascript"),"/static/styles.css":(ROOT/"ui"/"web"/"styles.css","text/css")}
        if path in routes:
    ...
    def do_PUT(self)->None:
        ...
        try:
            ...
            saved=sessions.save(sid,data.get("conversation",cur.get("conversation",[])),str(data.get("code",cur.get("code",""))),str(data.get("filename",cur.get("filename","main.py"))),str(data.get("detected_language",cur.get("detected_language","unknown"))),float(data.get("language_confidence",cur.get("language_confidence",0.0))),str(data.get("model",cur.get("model",""))),str(data.get("title",cur.get("title","New chat")))); self._send(200,saved)
        except Exception as exc: self._send(400,{"error":str(exc)})
    def do_DELETE(self)->None:
        path=urlparse(self.path).path
        if not path.startswith("/api/sessions/"): self._send(404,{"error":"not found"}); return
        self._send(200 if sessions.delete(path.rsplit("/",1)[-1]) else 404,{"deleted":True})
    def _handle_chat(self,data:dict[str,Any])->None:
        ...
        explicit_code=str(data.get("code",session.get("code",""))); prompt_code,prompt_tag=extract_prompt_code(request); code=explicit_code.strip() or prompt_code
        filename=str(data.get("filename",session.get("filename","main.py"))); model=str(data.get("model",session.get("model","qwen2.5-coder:7b"))); conversation=data.get("conversation",session.get("conversation",[]))
        ...
        turn=service_for(model).run_turn(request=request,code=code,language=detected,filename=filename,conversation=conversation)
        payload=turn.as_dict(); payload.update({"session_id":sid,"language":detected,"language_confidence":confidence,"language_evidence":evidence,"capabilities":service_for(model).capability_catalog()})
        if turn.success:
            updated=turn.conversation
            payload["session"]=sessions.save(sid,updated,turn.output_code or code,filename,detected,confidence,model)
    ...
    def _handle_feedback(self,data:dict[str,Any])->None:
        ...
        if feedback=="agree": self._send(200,{"status":"recorded","feedback":"agree","evolution":{"decision":"none","reasoning":"User accepted the result; no evolution analysis was required."}}); return
        event=evolution.record_disagreement(session_id=str(data.get("session_id","")),turn_id=int(data.get("turn_id",0)),request=str(data.get("request","")),language=str(data.get("language","unknown")),language_confidence=float(data.get("language_confidence",0.0)),previous_capability_id=str(data.get("capability_id","")),code=str(data.get("code","")))
...
class ReusableHTTPServer(ThreadingHTTPServer): allow_reuse_address=True
def main()->None:
    host,port="127.0.0.1",5000; print(f"SPS-CA dashboard: http://{host}:{port}"); print("Persistent Chat | Brain Auto Language | Explainable Evolution"); ReusableHTTPServer((host,port),Handler).serve_forever()
