...
class FakeBrain:
    ...
    def generate(self, request):
        return LLMResponse(text=json.dumps(self.plan), model=request.model or "test-model", provider=self.name)
...
def make_ui(tmp_path: Path) -> SPS_CA_Interface:
    return SPS_CA_Interface(history_path=tmp_path / "history.json", trace_history_path=tmp_path / "evolution_history.json", trace_stage_path=tmp_path / "stage_state.json")


def test_help_command_contains_required_commands(tmp_path: Path):
    text = make_ui(tmp_path).handle_command("help")
...
def test_show_registry_lists_canonical_capabilities(tmp_path: Path):
    response = make_ui(tmp_path).handle_command("show registry")
...
def test_process_request_uses_brain_plan_without_prompt_capability(tmp_path: Path):
    project = make_project(tmp_path)
    brain = FakeBrain({"intent": "fix syntax", "reasoning": "The source has a syntax error.", "steps": [{"capability_id": "CAP-002", "reason": "the source has a syntax error"}]})
...
def test_process_request_does_not_override_brain_with_keyword_rules(tmp_path: Path):
    project = make_project(tmp_path)
    brain = FakeBrain({"intent": "perform syntax repair", "reasoning": "Brain-selected syntax repair.", "steps": [{"capability_id": "CAP-002", "reason": "brain selected syntax repair"}]})
    ui = SPS_CA_Interface(history_path=tmp_path / "history.json", llm_provider=brain)
    ui.load_project(str(project))
