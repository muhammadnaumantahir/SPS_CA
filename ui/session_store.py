...
class SessionStore:
    ...
    def save(self, session_id: str, conversation: list[dict[str, str]], code: str, filename: str, language: str, language_confidence: float, model: str, title: str | None = None) -> dict[str, Any]:
        data = self._load()
        session = data["sessions"].get(session_id)
        if session is None:
            session = self.create(title or "New chat")
            data = self._load()
        session.update({
            "updated_at": self._now(),
        ...
        if title:
            ...
            if user_turns:
                session["title"] = user_turns[0].strip().replace("\n", " ")[:60] or "New chat"
    ...
    def _summary(session: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "preview": (last or {}).get("content", "")[:100],
