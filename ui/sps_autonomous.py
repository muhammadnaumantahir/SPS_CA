    ...
    def run_submission(
        ...
        *,
        ...
        scenario = self.trace_store.start_scenario(
            ...
            language=language,
            file_path=file_path,
            metadata={"source": "autonomous_sps_brain"},
        )
        scenario_id = scenario["scenario_id"]
        stage = int(scenario["stage_before"])
        workspace, relative_file = self._prepare_workspace(
            scenario_id=scenario_id, code=code, language=language,
            file_path=file_path, target_project=target_project,
        )
        ...
        def capabilities() -> list[dict[str, Any]]:
            return [
                {"id": c.id, "name": c.name, "description": c.description,
                 "status": c.status, "generated": bool(c.generated),
                 "allowed_intents": list(getattr(c, "allowed_intents", []) or []),
        ...
        def create_capability(request: str) -> dict[str, Any]:
            generated = evolution.plan_capability_for_gap(
                task_description=request,
                language=language.lower(),
                reason="Brain identified an unmet reusable capability requirement.",
                task_id=scenario_id,
            )
            governance = GovernanceGate().make_decision(
                change_id=f"evolution_{generated.capability_id}",
                change_type=ChangeType.EVOLUTION,
                change_description=generated.description,
                affected_files=[str(evolution.generated_dir / generated.capability_id.lower().replace("-", "_") / n) for n in ("capability.py", "tests.py", "metadata.json")],
                related_capabilities=[generated.capability_id],
            )
            ...
            outcome = evolution.develop_capability_for_gap(
                generated, project_root=str(self.repo_root), governance_decision_status=governance.decision,
            )
            if not outcome.get("registered"):
                raise RuntimeError(f"Layer 8 failed quality gates for {generated.capability_id}")
            return {"capability_id": generated.capability_id, **outcome}
        ...
        try:
            ...
            while pending and len(task_results) < max_iterations:
                ...
                if capability is None:
                    ...
                    decision = controller.reflect(
                        goal=user_request, decision=SPSDecision("reuse", "missing capability"), observation=observation,
                        current_code=current_code, remaining_tasks=[t.as_dict() for t in pending], iteration=len(task_results) + 1,
                    )
                ...
                record: dict[str, Any] = {"task_id": task.id, "intent_class": task.intent_class,
                    "capability_id": task.capability_id, "instruction": task.instruction,
                    "depends_on": list(task.depends_on), "success": result.success,
                ...
                if not result.success:
                    ...
                    observations.append(observation)
                    decision = controller.reflect(goal=user_request, decision=SPSDecision("reuse", "capability failed"),
                        observation=observation, current_code=current_code,
                        remaining_tasks=[t.as_dict() for t in pending], iteration=len(task_results))
                ...
                if result.modified_code is not None:
                    ...
                    if validation.status.value != "success":
                        ...
                        observations.append(observation)
                        decision = controller.reflect(goal=user_request, decision=SPSDecision("reuse", "validation failed"),
                            observation=observation, current_code=current_code,
                            remaining_tasks=[t.as_dict() for t in pending], iteration=len(task_results))
                        if len(task_results) < max_iterations:
                            if decision.strategy == "create":
                            ...
                            plan = make_plan(); pending = list(plan.tasks); continue
                        break
                    governance = GovernanceGate().make_decision(change.change_id, self._change_type({}, task.instruction),
                        task.instruction, [relative_file], related_capabilities=[task.capability_id])
                    record["governance"] = governance.decision.value
                    if governance.decision not in {DecisionStatus.AUTO_APPROVED, DecisionStatus.APPROVED}:
                        task_results.append(record); break
                    dna = self.dna.check_action(task.instruction, affected_files=[relative_file], require_rollback=True,
                        validated=True, governed=True, sandboxed=True, explicit_user_request=True)
                    record["dna"] = {"allowed": dna.allowed, "checked_rule_ids": dna.checked_rule_ids, "warnings": dna.warnings}
                    if not dna.allowed:
                        task_results.append(record); break
                    execution = ExecutionEngine(snapshot_dir=str(workspace / ".sps_snapshots"),
                        log_path=str(workspace / "execution_log.json"), registry=cap_registry).execute_change(change, str(workspace))
                    record["execution"] = execution.status.value; record["change_id"] = execution.change_id
                    if execution.status != ExecutionStatus.SUCCESS:
                        record["error"] = execution.error_message or "Execution failed"
                        task_results.append(record)
                ...
                reflection = controller.reflect(goal=user_request, decision=SPSDecision("reuse", "step completed"),
                    observation={"status": "completed", **record}, current_code=current_code,
                    remaining_tasks=[t.as_dict() for t in pending], iteration=len(task_results))
                if reflection.strategy == "finish" and not pending:
                    break
                if reflection.strategy == "create" and len(task_results) < max_iterations:
                    outcome = create_capability(reflection.task_instruction or user_request)
                    observations.append({"event": "capability_created", **outcome})
            ...
            self.trace_store.complete_scenario(
                ...
                result={"success": complete, "iterations": len(task_results), "composition": composition},
            )
            return {"scenario_id": scenario_id, "stage_before": stage, "stage_after": stage,
                    "success": complete, "brain_plan": plan.as_dict() if plan else {},
                    "task_results": task_results, "task_count": len(task_results),
                    ...
                    "brain": {"provider": controller.provider_name, "closed_loop": True,
                              "iterations": len(task_results), "observations": observations}}
