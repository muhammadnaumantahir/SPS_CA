...
class EvolutionEvidenceStore:
    ...
    @staticmethod
    def _next_id(events: list[dict[str, Any]]) -> str: return f"EVOL-{len(events) + 1:05d}"

    def record_agreement(self, *, session_id, turn_id, request, language, capability_id="", code=""):
        events = self._load()
        evidence = {"event_id": self._next_id(events), "event_type": "agreement", "timestamp": self._now(), "session_id": session_id, "turn_id": turn_id, "request": request, "language": language, "capability_id": capability_id, "code_length": len(code or ""), "evidence_summary": f"User accepted the result for {capability_id or 'the current turn'}; retained as positive reuse evidence.", "decision": "retain", "reasoning": "Positive feedback strengthens confidence in the selected behavior but does not justify creating a new capability."}
    ...
    def analyze(self, event):
        ...
        count = int(event.get("disagreement_count") or 0); parent = event.get("previous_capability_id") or ""; pattern = bool(event.get("failure_pattern"))
        decision = self.growth_decision.decide(existing_capability_id=parent, disagreement_count=count, capability_match=bool(parent), repeated_pattern=count >= 3 and pattern, adaptation_viable=count == 2, composition_viable=False, improvement_viable=False, capability_fitness=event.get("capability_fitness"), recurrence=event.get("recurrence_score"), adaptation_viability=event.get("adaptation_viability"), improvement_viability=event.get("improvement_viability"), composition_viability=event.get("composition_viability"), creation_need=event.get("creation_need"), confidence=event.get("confidence_score"), regression_risk=event.get("regression_risk"), evidence={"source_event_id": event.get("event_id", ""), "failure_pattern_detected": pattern})
        events = self._load(); result = dict(event)
        result.update({"decision": decision.decision.value, "reason_code": decision.reason_code, "reasoning": decision.reasoning, "growth_decision": {"decision": decision.decision.value, "reason_code": decision.reason_code, "reasoning": decision.reasoning, "evidence": decision.evidence, "scores": decision.scores}, "validation_status": "pending" if decision.decision.value == "create" else "not_applicable", "event_type": "evolution_analysis", "event_id": self._next_id(events), "timestamp": self._now()})
        events.append(result); self._save(events); return result

    def record_creation(self, analysis):
        events = self._load(); existing = [int(c.id.split("-")[-1]) for c in self.registry.list_all_capabilities() if c.id.startswith("CAP-") and c.id.split("-")[-1].isdigit()]; next_num = max([10, *existing]) + 1; cap_id = f"CAP-{next_num:03d}"; parent = analysis.get("previous_capability_id") or None; name = self._capability_name(analysis.get("request", ""), parent, cap_id)
        provenance = {"decision": "create", "created_at": self._now(), "parent_capability_id": parent, "trigger_event_ids": [analysis.get("event_id", "")], "reasoning": analysis.get("reasoning", ""), "reason_code": analysis.get("reason_code", "capability_gap"), "evidence_summary": analysis.get("evidence_summary", ""), "validation_status": "registered", "source_request": analysis.get("request", "")}
        metadata = {"id": cap_id, "name": name, "description": f"Generated reusable skill for the repeated pattern: {analysis.get('failure_pattern', 'observed user requirement')}", "type": CapabilityType.MODIFICATION.value, "entry_point": "capabilities.generated.evolved_runtime.run", "supported_languages": [analysis.get("language") or "python"], "version": "1.0.0", "created_date": self._now(), "last_modified": self._now(), "generated": True, "origin": "capability_evolution", "failure_pattern": analysis.get("failure_pattern"), "trigger_tasks": [str(analysis.get("session_id", ""))], "reuse_count": 0, "test_coverage": 0.0, "status": "active", "canonical": False, "extra_metadata": {"provenance": provenance, "tags": ["evolved", "explainable"], "decision_scores": analysis.get("growth_decision", {}).get("scores", {})}}
        self.registry.register_from_dict(metadata); creation = dict(analysis); creation.update({"event_type": "capability_created", "event_id": self._next_id(events), "timestamp": self._now(), "created_capability_id": cap_id, "validation_status": "registered", "capability_name": name, "provenance": provenance}); events.append(creation); self._save(events); return creation
    ...
    def get_capability_lineage(self, capability_id):
        ...
        if not provenance and related:
            created = related[-1]; provenance = dict(created.get("provenance") or {}); provenance.setdefault("decision", "create"); provenance.setdefault("created_at", created.get("timestamp")); provenance.setdefault("parent_capability_id", created.get("previous_capability_id") or None); provenance.setdefault("trigger_event_ids", [created.get("event_id", "")]); provenance.setdefault("reasoning", created.get("reasoning", "")); provenance.setdefault("evidence_summary", created.get("evidence_summary", "")); provenance.setdefault("validation_status", created.get("validation_status", "registered")); provenance.setdefault("source_request", created.get("request", ""))
