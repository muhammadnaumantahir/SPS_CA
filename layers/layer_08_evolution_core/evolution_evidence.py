...
class EvolutionEvidenceStore:
    ...
    def record_disagreement(
        ...
        *,
        ...
        evidence = {
            ...
            "failure_pattern": self._failure_pattern(request, code),
            ...
            "evidence_summary": evidence_summary or self._evidence_summary(previous_capability_id, count),
        }
        events.append(evidence)
        self._save(events)
        return evidence
    ...
    def analyze(self, event):
        ...
        decision = self.growth_decision.decide(
            ...
            improvement_viable=bool(event.get("improvement_viability") is not None and float(event.get("improvement_viability") or 0) >= 65),
        ...
        result.update({
            ...
            "validation_status": "pending" if decision.decision == GrowthDecision.CREATE else "not_applicable",
    ...
    def record_creation(self, analysis):
        ...
        existing = [
            int(c.id.split("-")[-1])
            for c in self.registry.list_all_capabilities()
            if c.id.startswith("CAP-") and c.id.split("-")[-1].isdigit()
        ]
        ...
        provenance = {
            "decision": GrowthDecision.CREATE.value,
            "created_at": self._now(),
            "parent_capability_id": parent,
            "trigger_event_ids": [analysis.get("event_id", "")],
            "reasoning": analysis.get("reasoning", ""),
            "reason_code": analysis.get("reason_code", "capability_gap"),
        ...
        metadata = {
            "id": cap_id,
            "name": name,
            "description": f"Generated reusable skill for the repeated pattern: {analysis.get('failure_pattern', 'observed user requirement')}",
            "type": CapabilityType.MODIFICATION.value,
            "entry_point": "capabilities.generated.evolved_runtime.run",
            "supported_languages": [analysis.get("language") or "python"],
            "version": "1.0.0",
            ...
            "trigger_tasks": [str(analysis.get("session_id", ""))],
            "reuse_count": 0,
            "test_coverage": 0.0,
            "status": "active",
            "canonical": False,
    ...
    def get_capability_lineage(self, capability_id):
        ...
        if not provenance and related:
            ...
            provenance.setdefault("decision", GrowthDecision.CREATE.value)
        ...
        return {
            ...
            "provenance": provenance,
            "events": related,
            "parent": provenance.get("parent_capability_id"),
        }
